...
- (void)application:(UIApplication *)application handleWatchKitExtensionRequest:(NSDictionary *)userInfo reply:(void(^)(NSDictionary *replyInfo))reply {
    ...
    // userInfoでkey値を受け取る Key:"counterValue" NSDictionary型
    NSString *counterValue = [userInfo objectForKey:@"counterValue"];
    
    // NSDictionaryインスタンスの生成 key:"response" value（値）:counterValue
    NSDictionary *replyDict = @{@"response": counterValue};
    
    // WatchKit App に返す NSDictionary型
    reply(replyDict);
    
    // ViewController countLabel に新しいカウンタ値を追加する
    AppDelegate *tmpDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    
    // ViewControllerとDelegateとの接続
    ViewController *vc = (ViewController *)(tmpDelegate.window.rootViewController);

    // countLabelにセット
    [vc.countLabel setText:counterValue];
}
